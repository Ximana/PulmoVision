...
class DetectorDoencasPulmonares:
    def __init__(self):
        ...
        self.img_width = 224
        self.model = None
        self.classes = ['normal', 'pneumonia', 'tuberculose']
        self.loaded = False
    ...
    def processar_imagem(self, imagem_file):
        try:
            if hasattr(imagem_file, "seek"):
                imagem_file.seek(0)
...
def analisar_radiografia(radiografia_obj):
    ...
    response = requests.get(radiografia_obj.imagem.url, timeout=10)
    response.raise_for_status()

    imagem_bytes = BytesIO(response.content)

    # Passar objeto file-like para o detector
    resultado = detector.detectar_doenca(imagem_bytes)
...
def gerar_deteccao_pdf(deteccao):
    ...
    elements.append(Paragraph(f"RESULTADO: {deteccao.diagnostico} ({deteccao.probabilidade}%)", resultado_style))
    elements.append(Spacer(1, 0.2*inch))
    
    # Tentar adicionar imagem da radiografia
    try:
        response = requests.get(deteccao.radiografia.imagem.url, timeout=10)
        response.raise_for_status()

        img_buffer = BytesIO(response.content)
        img = ReportImage(img_buffer, width=4*inch, height=4*inch)
